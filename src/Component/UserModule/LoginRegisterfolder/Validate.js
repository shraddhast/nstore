export default function validateInfo(valid){
    ...
    } else if (! /^[A-Za-z]+$/.test(valid.firstName)){
       errors.firstName = "Only Alphabets are allowed"
    }
   ...
   if(valid.lastName === "") {
   ...
   } else if (! /^[A-Za-z]+$/.test(valid.lastName)){
      errors.lastName = "Only Alphabets are allowed"
   }
    ...
    } else if(!/^(?=.*\d)(?=.*[a-z]).{4,14}$/.test(valid.cPassword)){
        errors.cPassword = "Password should be Alphanumeric"
    } else if((valid.cPassword.length <= 8) || (valid.cPassword.length > 12)){
           errors.cPassword ="Password length should be between 8-12"
    }

    if(valid.cPassword !== valid.cPassword){
